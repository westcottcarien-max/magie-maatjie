  ...
  useEffect(() => {
    async function load() {
      ...
      if (planData) {
        ...
        const sorted = (planData.plan_meals ?? [])
          .filter(pm => pm.meal_id)
    ...
    try {
      if (activePlan) {
        ...
        if (selectedMealIds.length > 0) {
          const rows = selectedMealIds.map((mid, i) => ({ plan_id: activePlan.id, day_index: i, meal_id: mid }))
        ...
        if (selectedMealIds.length > 0) {
          const rows = selectedMealIds.map((mid, i) => ({ plan_id: plan.id, day_index: i, meal_id: mid }))
          await supabase.from('plan_meals').insert(rows)
        }
        setActivePlan(plan)
